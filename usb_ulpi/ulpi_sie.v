...
//desrom256 rom(.address(romaddr), .clock(clk), .q(romq));
wire[7:0] romnum;
wire[8:0] romnum2;
		...
		begin
			if (en)
				case (cmd[7:6])
					...
					2'b10:begin // write reg
						state<=REGWT1;
						busy<=1;
						end
					default:state<=STATE_IDLE;
				endcase
			else if ((!full_rx && dir && nxt) || (!full_rx && dir && !nxt && indata[5:4] == 2'b01)) // see table 6-5, page 27
				state <= RXSTART;
			else 
				state <= STATE_IDLE;
		end
		...
		SETUPDATA16:begin
				...
				begin
					...
					begin
						...
						setupdatanum <= setupdatanum+1'b1;
						if (setupdatanum==8'd8) 
							crc16regr[7:0]<=indata;
						else 
							if(setupdatanum==8'd9)
							begin
